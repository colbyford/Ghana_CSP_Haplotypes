...
CSP_data <- read_csv("../CSP_sequence_analysis/finalTab_csp.csv") %>%
  ...
  filter(expression > 0) %>%
  group_by(SampleID) %>% 
  mutate(moi = length(unique(Haplotype))) #%>% 
...
comparison_data <- CSP_data %>%
  ...
  select(-c(weighted_HLA_CSP_HADDOCK, weighted_HLA_TCR_HADDOCK))
  # filter(HLA_CSP_HADDOCK == min(HLA_CSP_HADDOCK))
  # filter(expression == max(expression))
