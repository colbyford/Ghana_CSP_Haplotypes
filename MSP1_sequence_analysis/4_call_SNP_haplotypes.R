...
names(snpLst) <- "msp1"
saveRDS(snpLst, file.path(outputDir, sprintf("snpLst_msp%s.RDS", postfixSNP)))
# snpLst <- readRDS(file.path(outputDir, sprintf("snpLst_msp%s.RDS%s.txt", postfixSNP))


## Add in reference SNPs
# refSNP <- list(msp=read.delim("meta_data/refSNP_msp.txt"))

# snpLst[["msp"]] <- rbind(snpLst[["msp"]],
#                          refSNP[["msp"]] %>% 
#                            filter(!Pos %in% snpLst[["msp"]]$Pos)) %>%
#   arrange(Pos)
...
finalTab <- createFinalHaplotypTable(outputDir = outputDir,
                                     sampleTable = procReads[procReads$MarkerID=="msp1",],
                                     # sampleTable = procReads[(procReads$MarkerID=="msp1") & (procReads$SampleID != "Yakubu-1_S13"),],
                                     markerTable = markerTab["msp1",],
                                     referenceSequence = refSeq["msp1",],
...
saveRDS(finalTab, "finalTab_msp.RDS")
write.csv(do.call(rbind, finalTab), "finalTab_msp.csv", row.names = FALSE)
